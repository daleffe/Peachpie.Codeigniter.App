...
class CI_Form_validation {
	...
	public function __construct($rules = array())
	{
		...
		if (function_exists('mb_internal_encoding')) mb_internal_encoding($this->CI->config->item('charset'));
	...
	public function set_rules($field, $label = '', $rules = '')
	{
		...
		if (count($this->_post_fields) == 0) return $this;
		...
		if (is_array($field)) {
			foreach ($field as $row) {
				...
				if ( ! isset($row['field']) OR ! isset($row['rules'])) continue;
		...
		if ( ! is_string($field) OR  ! is_string($rules) OR $field == '') return $this;
		...
		if (strpos($field, '[') !== FALSE AND preg_match_all('/\[(.*?)\]/', $field, $matches)) {
			...
			for ($i = 0; $i < count($matches['0']); $i++) if ($matches['1'][$i] != '') $indexes[] = $matches['1'][$i];
	...
	{
		if ( ! is_array($lang)) $lang = array($lang => $val);
	...
	{
		if ( ! isset($this->_field_data[$field]['error']) OR $this->_field_data[$field]['error'] == '') return '';
	...
	public function error_string($prefix = '', $suffix = '')
	{
		...
		foreach ($this->_error_array as $val) if ($val != '') $str .= $prefix.$val.$suffix."\n";
	...
	public function run($group = '')
	{
		...
		if (count($this->_field_data) == 0) {
			...
			if (count($this->_config_rules) == 0) return count($this->_post_fields) > 0;
			...
			$global_rules = array_filter($this->_config_rules, function($key) { return !is_string($key); }, ARRAY_FILTER_USE_KEY);

			$this->set_rules(array_merge(is_null($global_rules) ? array() : $global_rules,$uri_rules,$group_rules));
		...
		foreach ($this->_field_data as $field => $row) {
			...
			if ($row['is_array'] == TRUE) {
				...
				if (isset($_POST[$field]) AND $_POST[$field] != "") $this->_field_data[$field]['postdata'] = $_POST[$field];
			}
		...
		foreach ($this->_field_data as $field => $row) {
			if ( ! is_null($row['postdata'])) {
				if ($row['is_array'] == FALSE) {
					...
					} else { foreach ($row['keys'] as $val) $post_ref =& $post_ref[$val]; }

					if (is_array($row['postdata'])) {
						$array = array();

						foreach ($row['postdata'] as $k => $v) $array[$k] = $this->prep_for_form($v);

						$post_ref = $array;
					} else { $post_ref = $this->prep_for_form($row['postdata']); }
				}
			}
		}
	}
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		if (is_null($postdata) AND $callback == FALSE) {
			if (in_array('isset', $rules, TRUE) OR in_array('required', $rules)) {
				...
				if ( ! isset($this->_error_messages[$type])) {
					if (FALSE === ($line = $this->CI->lang->line($type))) $line = 'The field was not set';
				} else { $line = $this->_error_messages[$type]; }
			...
			} else if (stripos($row['rules'], 'at_least_one') === false) { return; }
		}
		...
		foreach ($rules as $rule) {
			...
			if ($row['is_array'] == TRUE AND is_array($this->_field_data[$row['field']]['postdata'])) {
				...
				$_in_array = TRUE;
			} else { $postdata = $this->_field_data[$row['field']]['postdata']; }
			...
			if ($callback === TRUE) {
				...
				} else { $this->_field_data[$row['field']]['postdata'] = (is_bool($result)) ? $postdata : $result; }
				...
				if ( ! method_exists($this, $rule)) {
					...
					} else { log_message('debug', "Unable to find validation rule: ".$rule); }
			...
			if ($result === FALSE) {
				if ( ! isset($this->_error_messages[$rule])) {
					if (FALSE === ($line = $this->CI->lang->line($rule))) $line = 'Unable to access an error message corresponding to your field name.';
				} else { $line = $this->_error_messages[$rule]; }

				// Is the parameter we are inserting into the error message the name
				// of another field?  If so we need to grab its "field label"
				if (isset($this->_field_data[$param]) AND isset($this->_field_data[$param]['label'])) $param = $this->_translate_fieldname($this->_field_data[$param]['label']);
	...
	protected function _translate_fieldname($fieldname)
	{
		...
		if (substr($fieldname, 0, 5) == 'lang:') {
			...
			if (FALSE === ($fieldname = $this->CI->lang->line($line))) return $line;
		}
	...
	{
		if ( ! isset($this->_field_data[$field])) return (isset($_POST[$field]) ? $_POST[$field] : $default);

		// If the data is an array output them one at a time.
		//     E.g: form_input('name[]', set_value('name[]');
		if (is_array($this->_field_data[$field]['postdata'])) return array_shift($this->_field_data[$field]['postdata']);
	...
	{
		if ( ! isset($this->_field_data[$field]) OR ! isset($this->_field_data[$field]['postdata'])) {
			if ($default === TRUE AND count($this->_field_data) === 0) return ' checked="checked"';
			return '';
		}
	...
	public function is_unique($str, $field)
	{
		...
		if (count($fields) > 0) {
			...
			if (count($table_fields) > 0) {
				...
				if (count($table_fields) == 2) {
					...
					if (isset($_POST[$table_fields[1]])) $diff[$table_fields[0]] = $_POST[$table_fields[1]];
				}
			}
		}
		...
		foreach ($fields as $field_collection) {
			...
			if (count($field_split)  == 2) if (isset($_POST[$field_split[1]])) $values[trim($field_split[0])] = trim($_POST[$field_split[1]]);
		}

		if (!empty(trim($table)) && !empty(trim($field))) {
			$filter = array($field => $str);

			if (count($diff) == 1) $filter[trim(key($diff)) . ' != '] = trim($diff[key($diff)]);

			foreach ($values as $value_key => $value_val) $filter[trim($value_key)] = trim($value_val);
	...
	public function valid_emails($str)
	{
		...
		foreach (explode(',', $str) as $email) if (trim($email) != '' && $this->valid_email(trim($email)) === FALSE) return FALSE;
	...
	{
		return ( ! is_numeric($str)) ? FALSE : $str > $min;
	}
	...
	{
		return ( ! is_numeric($str)) ? FALSE : $str < $max;
	}
	...
	{
		if (is_array($data)) {
			foreach ($data as $key => $val) $data[$key] = $this->prep_for_form($val);

			return $data;
		}

		if ($this->_safe_form_data == FALSE OR $data === '') return $data;
